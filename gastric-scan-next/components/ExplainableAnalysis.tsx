...
export const ExplainableAnalysis: React.FC<ExplainableAnalysisProps> = ({
  ...
  const renderMetricGauge = (value: number, label: string, sublabel: string, thresholds: [number, number], icon: React.ReactNode) => {
    ...
    return (
      <div className="flex flex-col items-center p-4 bg-[#0d0d0d] rounded-xl border border-white/5">
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md ${isFullscreen ? 'p-0' : 'p-4'}`}>
      <div className={`bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden flex flex-col shadow-2xl transition-all duration-300 ${
        isFullscreen ? 'w-full h-full rounded-none' : 'w-[95vw] max-w-7xl max-h-[95vh]'
      }`}>
        {/* Header - 科研风格 */}
        <div className="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-gradient-to-r from-[#0d0d0d] to-[#111]">
          ...
          {result && result.success && (
            <div ref={reportRef} className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                {/* 左侧：可视化图像 */}
                <div className="xl:col-span-2 bg-[#0d0d0d] rounded-2xl border border-white/5 overflow-hidden">
                ...
                <div className="space-y-4">
                  {/* 三大指标 */}
                  <div className="bg-[#0d0d0d] rounded-2xl border border-white/5 p-5">
